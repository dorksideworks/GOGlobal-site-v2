...
function Downloads2({sendTheme}) {
    ...
    const handlePresDownload = async (s) => {
        if(selectedPres) {
            console.log('https://cms.goglobal.network'+selectedPres.file.data.attributes.url);
            console.log(selectedPres);
            await axios({
                url: 'https://cms.goglobal.network'+selectedPres.file.data.attributes.url,
                method: 'GET',
                ...
                link.href = url;
                link.setAttribute('download', selectedPres.file.data.attributes.name);
    ...
    return (
        <div class="pt-52 relative bg-slate-50 duration-500 relative ">
            ...
            <div id="downloads-section" class="wrapper mx-auto">
                ...
                <h1 class="text-6xl font-bold py-5  "> Downloads Center <br/> 
                        ...
                        return (
                            <div key={index} class="downloads-card p-7 rounded-xl flex flex-col group basis-1/3">
...

                                    {
                                        ...
                                        <select onChange={e => {setSelectedPres(item.attributes.files[e.target.value]); console.log(selectedPres)}} class="mt-5 p-2 rounded-md self-start justify-self-start font-bold text-sm">
                                            <option > Please Select </option>
                                            {
                                                item.attributes.files.map(function(doc, x) {
                                                    return(<option value={x} > {doc.file.data.attributes.name} </option>);
                                                })
                                            }
