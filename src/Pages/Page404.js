import React, {useEffect, useRef, useState} from 'react'

import axios from 'axios';

import { i18n, useTranslation } from 'react-i18next';

const token = "";

function Page404({sendTheme}) {
   
    const [selectedPres, setSelectedPres] = useState('');



    useEffect(() => {
        sendTheme('dark');

        let param = {}
        
    },[]);


    return (
        <div class="pt-52 relative bg-slate-900 duration-500 relative bg-404 ">

            <div id="downloads-section" class="wrapper mx-auto flex flex-col">
                    <h1 class="text-white text-6xl font-bold mb-10">Ooops! Are you lost in space?</h1>
                    <p class="text-white pb-10"> Unfortunately, the page you are looking for is currently not available. </p>
                    <img src="/ASSETS/404/404-hero.png"/>
                    <h1 class="text-white text-6xl"></h1>
                    <a href="/" class="py-2 px-5 bg-cyan-500 w-72 font-bold mx-auto rounded-full overflow-hidden"> HOMEPAGE</a>
            </div>

        </div>
    )
}

export default Page404
